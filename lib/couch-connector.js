...
exports.connector = Class.extend({
    ...
    var viewURL;
    if( params.custom_view_name ) {
      viewURL = this.buildCustomViewURL(params.custom_view_name,params.type);
    }
		else {		
  ...
  buildCustomViewName: function(name) {
	...
	buildViewParams: function(getParams) {
        ...
        if(key=='custom_view' || key=='custom_view_name') {
          continue;
        }
				if (i) {
					getParamsString += '&';
	...
	createView: function(params, callback) {
        ...
        // create custom view
        if(params.hasOwnProperty('custom_view') && params['custom_view']) {
          if(!params.custom_view_name) {
            // no name specified, send error back
            callback({message: 'Need to specify custom_view_name parameter'},null);
            return;
          }
          if(typeof(params.custom_view) != 'function') {
            callback({message:'custom_view must be a function'},null);
            return;
          }
          doc.views[_this.buildCustomViewName(params.custom_view_name)] = {
            map: params.custom_view.toString()
          };
        }
				else {
